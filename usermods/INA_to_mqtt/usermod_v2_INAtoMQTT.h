...
class UserMod_INAToMQTT : public Usermod
{
private:
  ...
  {
    mqttVoltageTopic = String(mqttDeviceTopic) + "/voltage";
    mqttPowerTopic = String(mqttDeviceTopic) + "/power";
    mqttCurrentTopic = String(mqttDeviceTopic) + "/current";
    ...
    _createMqttSensor("voltage", mqttVoltageTopic, "voltage", "mV");
    _createMqttSensor("power", mqttPowerTopic, "power", "mW");
    _createMqttSensor("current", mqttCurrentTopic, "current", "mA");
  ...
  void _updateSensorData()
  {
    ...
    INAVoltage = ina260.readBusVoltage();
    INACurrent = ina260.readCurrent();
    INAPower = ina260.readPower();
  ...
  void loop()
  {
    ...
    if (tempTimer > nextMeasure)
    {
      ...
      if (mqtt != nullptr && mqtt->connected())
      {
        ...
        mqtt->publish(mqttVoltageTopic.c_str(), 0, true, String(INAVoltage).c_str());
        mqtt->publish(mqttPowerTopic.c_str(), 0, true, String(INAPower).c_str());
        mqtt->publish(mqttCurrentTopic.c_str(), 0, true, String(INACurrent).c_str());
